...
from .database import engine, UnitOfWork
...
def listar_transacoes(uow: UnitOfWork = Depends(get_uow)):
    with uow.get_session() as db:
        return crud.get_transacoes(db)

@app.get("/transacoes/{transacao_id}", response_model=schemas.Transacao)
def buscar_transacao(transacao_id: int, uow: UnitOfWork = Depends(get_uow)):
    with uow.get_session() as db:
...
def criar_transacao(transacao: schemas.TransacaoCreate, uow: UnitOfWork = Depends(get_uow)):
    with uow.get_session() as db:
        return crud.create_transacao(db, transacao)

@app.delete("/transacoes/{transacao_id}")
def deletar_transacao(transacao_id: int, uow: UnitOfWork = Depends(get_uow)):
    with uow.get_session() as db:
...
def atualizar_transacao(transacao_id: int, transacao: schemas.TransacaoCreate, uow: UnitOfWork = Depends(get_uow)):
    with uow.get_session() as db:
...
def calcular_total(tipo: str, uow: UnitOfWork = Depends(get_uow)):
