...
async def signup(user:SignUpModel):
    ...
    if db_username is not None:
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Usuário já cadastrado")
...
async def login(user:LoginModel, Authorize: AuthJWT = Depends()):
    ...
    raise HTTPException(status_code=status.HTTP_401_BAD_REQUEST, detail="Usuário ou senha inválidos")
